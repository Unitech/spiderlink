...
Client.prototype.call = function(function_name, data, cb) {
  ...
  var rpc_id = crypto.randomBytes(32).toString('hex');

  this.publish(this.namespace + function_name, {
    data : data,
    rpc_id : rpc_id
  });
  var ns = self.namespace + function_name + ':' + rpc_id;
...
// Same as .call but does not unsubscribe at first event
Client.prototype.longCall = function(function_name, data, cb) {
  var self = this;
  if (typeof(data) == 'function') cb = data;
